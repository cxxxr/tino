...
static void init_serial(const uint16 port) {
  ...
  write_io_port8(port + 2,
                 0xC7); // Enable FIFO, clear them, with 14-byte threshold
  write_io_port8(port + 4, 0x0B); // IRQs enabled, RTS/DSR set
}

void init_serial_ports(void) { init_serial(PORT); }

static int serial_received(void) { return read_io_port8(PORT + 5) & 1; }
...
static int is_transmit_empty(void) { return read_io_port8(PORT + 5) & 0x20; }
...
void print_uint64(uint64 value) { print_uint64_with_padding(value, 0); }
